<?php
// File: check_daftar_pesanan.php

function checkDaftarPesanan($koneksi, $idUser) {
    // Escape the user input using mysqli_real_escape_string
    $idUser = mysqli_real_escape_string($koneksi, $idUser);

    $query = "SELECT id, waktu_pembuatan FROM daftar_pesanan WHERE user_id = '$idUser' ORDER BY waktu_pembuatan DESC";
...
// Panggil fungsi checkDaftarPesanan() dan dapatkan respons
$response = checkDaftarPesanan($koneksi, $idUser);
