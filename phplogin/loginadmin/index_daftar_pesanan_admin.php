...
<head>
    ...
    <title>Index Daftar Pesanan Admin</title>
...
<body>
    ...
    <ul>
        ...
        <a href="index_daftar_pesanan_admin.php" class="button-daftar-beda" id="daftarPesananButton">
...
function changeDefaultOption(selectElement) {
    ...
    const localStorageKey = `selectedStatusPesananAdmin_${selectElement.dataset.id}`; //selectedStatusPesananAdmin_ bisa divariasikan agar penyimpanan local storage nya berbeda-beda
    ...
    if (selectedIndex !== 0) {
        ...
        localStorage.removeItem(`selectedStatusPesananAdmin_${selectElement.dataset.id}`);
    }
}

// Mengambil data dari backend
fetch('cek_daftar_pesanan_admin.php')
    .then(response => response.json())
    .then(data => {
        // Menampilkan data
        data.forEach(row => {
            ...
            const selectedStatus = localStorage.getItem(`selectedStatusPesananAdmin_${row.id}`);
            if (selectedStatus) {
