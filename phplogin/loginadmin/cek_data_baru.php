...
if ($result->num_rows > 0) {
    ...
    $sql_old_count = "SELECT hasil_hitung FROM hasil_perhitungan_pesanan ORDER BY id DESC LIMIT 1";
    $result_old_count = $conn->query($sql_old_count);

    if ($result_old_count->num_rows > 0) {
        $row_old_count = $result_old_count->fetch_assoc();
        $jumlah_pesanan_lama = $row_old_count["hasil_hitung"];
        ...
        if ($jumlah_pesanan_lama < $jumlah_pesanan_terbaru) {
            ...
            $sql_update = "INSERT INTO hasil_perhitungan_pesanan (hasil_hitung) VALUES ('$jumlah_pesanan_terbaru')";
