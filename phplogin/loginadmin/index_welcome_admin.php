...
<body>
    ...
    <ul>
        ...
        <a href="index_welcome_admin.php" class="button-daftar-beda">
    ...
    <div class="note-input">
        ...
        <label for="note">Tambah Catatan Status Depot</label>
    </div>
    
    <div class="button-reset">
    <button class="reset-button" onclick="resetAndDeleteData()">Reset</button>
    </div>

    <div class="button-simpan">
        <!-- Panggil fungsi saveToDatabase saat tombol "Simpan" diklik -->
        <button class="reset-button" onclick="saveToDatabase()">Simpan</button>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function saveSelectedDateToDatabase(selectedDate) {
            // Ubah objek selectedDate menjadi string dengan format YYYY-MM-DD
            var formattedDate = selectedDate.year + '-' + selectedDate.month + '-' + selectedDate.day;

            // Buat objek payload untuk dikirim ke server
            var payload = {
                nilai_kalender: formattedDate
            };

            // Kirim permintaan POST ke server
            fetch('cek_nilai_kalender.php', {
                method: 'POST',
                headers: {
        ...
        // Modifikasi fungsi handleSelectChange
    ...
    <script>
        // Fungsi saveToDatabase untuk mengirim data ke server saat tombol "Simpan" diklik
        function saveToDatabase() {
            ...
            if (noteText.trim() === '') {
                // Menampilkan notifikasi jika textarea kosong
                showNotification('Data tidak boleh kosong!', 'error');
                return; // Menghentikan eksekusi lebih lanjut karena data tidak boleh kosong
            }

            // Mengirim data menggunakan metode POST ke file PHP server
            fetch('simpan_catatan_status_depot.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            .then(response => {
                if (response.ok) {
                    ...
                    showNotification('Anda Berhasil Membuat Status', 'success');

                    // Mengarahkan ke halaman welcome_admin-1.php setelah notifikasi muncul
                    window.location.href = 'index_welcome_admin-1.php';
                } else {
                    // Jika respons dari server gagal
                    showNotification('Anda Gagal Membuat Status', 'error');
                }
        ...
        function resetAndDeleteData() {
        // Reset textarea
        resetTextArea();

        // Hapus data dari tabel catatan_status_depot
        deleteDataFromDatabase();
        }

        function deleteDataFromDatabase() {
        // Menggunakan fetch untuk mengirim permintaan DELETE ke server
        ...
        function resetTextArea() {
            // Fungsi resetTextArea untuk mengatur nilai textarea menjadi kosong
        document.getElementById('note').value = '';

            // Fokuskan kembali ke textarea setelah direset
