...
<body>
    ...
    <ul>
        ...
        <a href="index_pengaturan_harga_admin.php" class="button-daftar-beda">
            ...
            <button class="button-atas-yang-terakhir" id="airBestariButton">Bestari</button>
                ...
                <div class="blue-card">
                    ...
                    <div class="quantity-container">
                        ...
                        <span class="label-jumlah">Harga:</span>

                        <input type="number" name="harga_galon" id="hargaInput" class="quantity-input" placeholder="Rp" min="0">
                    ...
                    <div class="submit-semua-admin">
                        ...
                        <div class="tombol-beli-container">
                            <button id="tombolSimpanAdmin" class="tombol-beli">Simpan</button>
                        </div>
                    </div>
    ...
    // Mengambil nilai harga_air_isi_ulang dari server saat halaman dimuat
    document.addEventListener('DOMContentLoaded', function() {
        // Menggunakan AJAX untuk mengambil nilai dari server (contoh dengan JavaScript Fetch API)
        fetch('get_harga-1.php')
            .then(response => response.json())
            .then(data => {
                // Menyimpan nilai harga_air_isi_ulang dari server ke dalam input
                hargaInput.value = data.harga_sanford;
            })
            .catch(error => console.error('Error:', error));
    });

// Mendapatkan elemen input untuk ditampikan dalam harga rupiah
// var hargaInput = document.getElementById('hargaInput');

// Mengambil nilai harga_air_isi_ulang dari server saat halaman dimuat
// document.addEventListener('DOMContentLoaded', function() {
    // Menggunakan AJAX untuk mengambil nilai dari server (contoh dengan JavaScript Fetch API)
    // fetch('get_harga-2.php')
    //     .then(response => response.json())
    //     .then(data => {
            // Menambahkan format "Rp" dan tanda "." pada nilai harga
            // var formattedHarga = 'Rp ' + formatNumber(data.harga_bestari);
            
            // Menyimpan nilai harga_air_isi_ulang yang sudah diformat ke dalam input
//             hargaInput.value = formattedHarga;
//         })
//         .catch(error => console.error('Error:', error));
// });

// Fungsi untuk memformat angka dengan tanda "." sebagai pemisah ribuan
// function formatNumber(number) {
//     return number.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".");
// }

    // Fungsi untuk menyimpan perubahan harga ke dalam database
    function simpanHarga() {
        // Mendapatkan nilai baru dari input
        var hargaBaru = hargaInput.value;

        // Pemeriksaan apakah nilai hargaBaru sama dengan 0
        if (hargaBaru == 0) {
            // Menampilkan alert bahwa nilai tidak boleh 0
            alert('Harga Pesanan tak mungkin bernilai 0!!!');
            return; // Menghentikan proses pengiriman
        }

        // Menggunakan AJAX untuk menyimpan nilai ke dalam database (contoh dengan JavaScript Fetch API)
        fetch('update_harga-1.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ harga_sanford: hargaBaru }),
        })
        .then(response => response.json())
        .then(data => {
            // Setelah berhasil menyimpan, Anda mungkin ingin memberikan feedback kepada pengguna
            alert('Harga berhasil disimpan: ' + data.harga_sanford);
        })
        .catch(error => console.error('Error:', error));
    }
    </script>
    ...
    <script>
        ...
        var tombolSimpanAdmin = document.getElementById("tombolSimpanAdmin");

        // Menambahkan event listener untuk menanggapi klik
        tombolSimpanAdmin.addEventListener("click", function() {
            // Memanggil fungsi simpanHarga ketika tombol "Simpan" ditekan
            simpanHarga();
        });
    </script>
    ...
    <script>
        // function resetInput() {
            ...
            // var inputElement = document.getElementById('hargaInput');

            // Mereset nilai input menjadi kosong
            // inputElement.value = '';
        // }
        function resetInput() {
        // Mendapatkan nilai harga_air_isi_ulang dari server saat halaman dimuat
        fetch('get_harga-1.php')
            .then(response => response.json())
            .then(data => {
                // Mengembalikan nilai input harga ke nilai aslinya dari server
                hargaInput.value = data.harga_sanford;
            })
            .catch(error => console.error('Error:', error));
        }
...
<script>
    ...
    airIsiUlangButtonButton.addEventListener("click", function() {
        ...
        window.location.href = "index_pengaturan_harga_admin.php";
    });
</script>
...
<script>
    ...
    sanfordButtonButton.addEventListener("click", function() {
        // Mengarahkan ke halaman "index_tambah_pesanan_admin.php"
        window.location.href = "index_pengaturan_harga_admin-1.php";
    });
</script>
...
<script>
    ...
    bestariButtonButton.addEventListener("click", function() {
        // Mengarahkan ke halaman "index_tambah_pesanan_admin.php"
        window.location.href = "index_pengaturan_harga_admin-2.php";
    });
</script>
